...
\caption{Arquitectura del sistema de comunicaciones}
\end{figure}
...
\subsection{Conexión Remota entre el Planificador Erlang y el Agente C}

Una extensión posterior de la consigna requirió que un planificador Erlang pudiera conectarse a un agente C ejecutándose en una máquina distinta, en lugar de restringirse únicamente a \texttt{localhost}. Para lograr esto sin perder la distinción entre tráfico público (entre agentes C) y tráfico local (Erlang hacia su agente), se adoptó un esquema de \textbf{dos puertos TCP consecutivos} por agente:

\begin{itemize}
    \item \texttt{<port>}: puerto público, utilizado para la comunicación entre agentes C (mensajes \texttt{RESERVE}, \texttt{GRANTED}, \texttt{DENIED}, \texttt{RELEASE}).
    \item \texttt{<port> + 1}: puerto local, reservado exclusivamente para la interfaz con el planificador Erlang.
\end{itemize}

Ambos sockets se vinculan a la dirección IP de la LAN del nodo (\texttt{ctx->lan\_ip}) en lugar de restringirse a \texttt{127.0.0.1}, permitiendo que un planificador Erlang remoto se conecte al puerto \texttt{<port>+1} de un agente en otra máquina de la misma red. La función \texttt{erlang\_c\_bridge:init/4} recibe el \texttt{Host} y el \texttt{Port} como parámetros explícitos (en lugar de macros fijas), lo que permite iniciar el planificador con:

\begin{lstlisting}
make runclient HOST=<ip_del_agente> PORT=<puerto_local>
\end{lstlisting}

Durante las pruebas de conexión remota se identificó que, si bien el descubrimiento UDP entre agentes funciona correctamente incluso con \emph{firewalls} activos (dado que UDP en modo broadcast suele estar permitido), las conexiones TCP entrantes pueden ser bloqueadas por el firewall del sistema operativo receptor. Esto se manifiesta como un \texttt{JOB\_TIMEOUT} en Erlang para trabajos que involucran un nodo remoto, aun cuando el \texttt{ping} entre las máquinas es exitoso. La solución consiste en habilitar explícitamente ambos puertos TCP (público y local) en el firewall de cada máquina participante; este procedimiento quedó documentado en el \texttt{README.md} del proyecto como parte de la guía de resolución de problemas.

\subsection{Tolerancia a Fallos y Enlace de Procesos}

Para cumplir con el requisito de que, ante la caída de un proceso auxiliar (\emph{worker}), el planificador completo también finalice, se utilizó el mecanismo de \textbf{enlace de procesos} (\texttt{link/1}) propio de Erlang. Cuando el planificador recibe los identificadores de proceso de \texttt{sender} y \texttt{receiver}, se enlaza a ambos:
...
%---------------------------------------------------------------
\section{Script de Prueba de Deadlock (\texttt{test\_deadlock.sh})}
...
La información de cada nodo se unifica en una única estructura opaca, \texttt{node\_data\_t}, que actúa como contenedor de los tres módulos descriptos en esta sección (\texttt{known\_nodes}, \texttt{local\_resources} y \texttt{owned\_jobs}). En lugar de utilizar un único \emph{mutex} global para proteger todo el estado del nodo, se optó por un esquema de \textbf{tres locks independientes}, uno por módulo:
