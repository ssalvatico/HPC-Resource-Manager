...
#include <time.h>
#include "queues.h"
#include "prototipos-estructuras.h"
#include "tablahash.h"
...
static petitions_t          create_petitions(){
    return tablahash_crear(11, copy_rec_petition, comp_rec_petition, dest_rec_petition, hash_rec_petition);
}
...
static owned_jobs_t         create_owned_jobs(){
    return tablahash_crear(11, copy_owned_job, comp_owned_job, dest_owned_job, hash_owned_job);
}
...
static local_resources_t * create_local_resource( unsigned cpu,  unsigned gpu,  unsigned ram){
    ...
    for( int t = 0 ; t < RES_NUM ; t++) { out -> avail[t] = out -> total[t]; out -> request_queue[t] = queue_create(); }
...
static void delete_local_resource(local_resources_t * resources){
    ...
    for (unsigned idx = 0 ; idx < RES_NUM ; idx++) { queue_destroy(resources -> request_queue[idx], dest_job_request); }
...
static unsigned new_job_request (local_resources_t * resources, active_jobs_t active_jobs, unsigned id, unsigned socket, unsigned quantity, resource_t type){
    if(resources -> avail[type] < quantity){
        resources -> request_queue[type] = queue_add(resources -> request_queue[type], create_job_request(id, socket, quantity, type), copy_job_request);
        return 0;
    }
...
static unsigned chk_job_request (local_resources_t * resources, active_jobs_t active_jobs){
    queue_t                 act         = NULL;
    elem_job_request_t  *   act_elem    = NULL;
    int out = 0;    

    for ( unsigned tries = 0, init = (rand() % RES_NUM), type; tries < RES_NUM && !out ; tries++){
        type = (init + tries) % RES_NUM;
        ...
        do{
            ...
            resources -> request_queue[type] = queue_delete(resources -> request_queue[type], act_elem, dest_job_request, comp_job_request);
...
static void     known_nodes_del_inactive_nodes(known_nodes_t nodes){
    cont = 0;
    ...
    tablahash_visitar(nodes, known_nodes_del_inactive);
...
static void known_nodes_get_data(known_nodes_t nodes, char * OUT_BUFFER, unsigned OUT_SIZE){
    ...
    tablahash_visitar(nodes, append_node);
...
static void update_granted_job_cond(elem_petition_t * petition){
    granted_job_cond = granted_job_cond && petition; 
}
...
static int check_job_granted(own_jobs_t jobs, unsigned job_id){
    ...
    for( unsigned type = 0 ; type < RES_NUM && granted_job_cond; type++){
        tablahash_visitar(act -> resource_petitions[type], update_granted_job_cond);
    }
...
static command_get_command(const char * BUFFER){
    char cmd[32];
    ...
    if  (!strcmp(cmd, "JOB_STATUS"))        return CMD_JOB_STATUS;
    return CMD_INVALID;
}
...
node_data_t node_init   (unsigned cpu, unsigned gpu, unsigned ram){
    ...
    node -> active_jobs = tablahash_crear(11, copy_active_job, comp_active_job, dest_active_job, hash_active_job);
    node -> known_nodes = tablahash_crear(11, copy_known_node, comp_known_node, dest_known_node, hash_known_node);
    node -> active_jobs = create_owned_jobs();
...
unsigned master_function(node_data_t NODE, char * NODE_IP, unsigned SOCKET, const char * BUFFER, char IP[16], char * OUT, unsigned OUT_SIZE){      // receives C agent's message, parses it and mannage data structures according to pettition
    ...
    switch(task){
        ...
        case CMD_JOB_RELEASE:   {
            ...
            if(out){
                elem_owned_job_t * dummy = create_owned_job(job_id);
                if(!tablahash_buscar(NODE -> owned_jobs, dummy)){
                    dest_owned_job(dummy);
                    return ERROR;
                }
